...
/* ========================================================================== */
/*  Detail row — a single <dt>/<dd> pair                                      */
/* ========================================================================== */
...
/* ========================================================================== */
/*  Section — a titled block inside a card body                              */
/* ========================================================================== */
...
export default async function ProductDetailPage({
  params,
}: PageProps<"/stores/[storeId]/owner/products/[productId]">) {
  const { storeId, productId: id } = await params;

  const result = await getProductById(storeId, id);

  /* The service returns AppError for both "not found" and validation
     failures. Either way, there's nothing to render — 404. */
  if (result instanceof AppError) notFound();

  const product = result;

  const base = `/stores/${storeId}/owner/products`;
                ...
                <DetailRow label="Margin per Unit">
                  <span
                    className={cn(
                      ...
                      marginAmount >= 0 ? "text-success-fg" : "text-danger-fg",
                    )}
                  >
        ...
        <div className="space-y-4">
            ...
            <CardBody>
              <DetailList>
                ...
                <DetailRow label="Minimum">
                  <span className="tabular-nums">{product.minimumStock}</span>
                </DetailRow>
          ...
          <Card>
            ...
            <CardBody className="space-y-2">
              <HistoryLink
                href={`/stores/${storeId}/owner/sales?productId=${product.id}`}
                label="Sales history"
              />
              <HistoryLink
                href={`/stores/${storeId}/owner/purchases?productId=${product.id}`}
